 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_gradingform_rubric_ranges extends behat_base {
    ...
    public function i_define_the_following_rubric(TableNode $rubric) {
        ...
        $addcriterionbutton = $this->find_button(get_string('addcriterion', 'gradingform_rubric_ranges'));

        // Cleaning the current ones.
        $deletebuttons = $this->find_all('css', "input[value='" . get_string('criteriondelete', 'gradingform_rubric_ranges') . "']");
        if ($deletebuttons) {
        ...
        if ($criteria) {
            foreach ($criteria as $criterionit => $criterion) {
                ...
                if (count($criterion) < 5) {
                    throw new ExpectationException(
                        get_string('err_mintwolevels', 'gradingform_rubric_ranges'),
                        $this->getSession()
                    );
